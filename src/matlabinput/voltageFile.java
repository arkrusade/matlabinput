...
//            double[] isSpiked = new double[voltages.length];
        ...
        //0 is not spike, 1 is spike started, 2 is from first peak to second peak, 3 is from second peak to reverse
        for (int i = 1; i < voltages.length; i++) {
            ...
            if (absD >= .005) {//if diff is big (either direction)
                ...
                if (step == 3) {
                    step++;
                    findingSpikes.get(findingSpikes.size() - 1).setStep(4, i);//1 for first peak, 2 for second peak
                } else if (step == 1 && d < 0 == upwards) {//to go to step 2, must be step 1 and switch directions
                    step = 2;
                    //every check
                    //looking for small diff
                    //delta must be smaller than some other diff
                    //in current spike
                } else if (step == 2 && (d > 0 == upwards && d10 > 0 == upwards)) {//to go to step 3, must be in same direction
                    step = 5;
                }
            }
            steps[i] = (double) step / 100.0;
